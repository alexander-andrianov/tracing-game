...
namespace PathCreation {
    ...
    public class VertexPath {
        ...
        public (Vector3, int) GetClosestPointAndIndexOnPath (Vector3 worldPoint) {
            var data = CalculateClosestPointAndIndexOnPathData (worldPoint);
            return (GetPoint(data.Item1.nextIndex), data.Item2);
        }
        ...
        public float GetClosestTimeOnPath (Vector3 worldPoint) {
            var data = CalculateClosestPointAndIndexOnPathData (worldPoint);
            return Mathf.Lerp(times[data.Item1.previousIndex], times[data.Item1.nextIndex], data.Item1.percentBetweenIndices);
        }
        ...
        public float GetClosestDistanceAlongPath (Vector3 worldPoint) {
            var data = CalculateClosestPointAndIndexOnPathData (worldPoint);
            return Mathf.Lerp(cumulativeLengthAtEachVertex[data.Item1.previousIndex], cumulativeLengthAtEachVertex[data.Item1.nextIndex], data.Item1.percentBetweenIndices);
        }
        ...
        (TimeOnPathData, int) CalculateClosestPointAndIndexOnPathData (Vector3 worldPoint) {
            ...
            for (var i = 0; i < localPoints.Length; i++) {
                ...
                var closestPointOnSegment = MathUtility.ClosestPointAndIndexOnLineSegment (worldPoint, GetPoint (i), GetPoint (nextI), i, nextI);
                var sqrDst = (worldPoint - closestPointOnSegment.Item1).sqrMagnitude;
                
                if (sqrDst < minSqrDst) {
                    minSqrDst = sqrDst;
                    closestPoint = closestPointOnSegment.Item1;
                    closestIndex = closestPointOnSegment.Item2;
            ...
            return (new TimeOnPathData(closestSegmentIndexA, closestSegmentIndexB, t), closestIndex);
        }
